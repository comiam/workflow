...
final class Module_string {
	...
	Struct f_doStrSplit(String as, String asep, int astart, Struct aacc) {
	 TAIL_CALL: for(;;) {
		final int l0_l = runtime.h_Native.strlen(as);
		final int l1_i = ((int)runtime.n_strRangeIndexOf.invoke(as, asep, ((Integer)astart), ((Integer)l0_l)));
		if ((l1_i<0)) {
			return (new Struct_Cons(runtime.h_Native.substring(as, astart, l0_l), aacc));
		} else {
			final String l2_part = runtime.h_Native.substring(as, astart, (l1_i-astart));
			final int l3_sepLen = runtime.h_Native.strlen(asep);
			{
	...
	boolean f_isDigit(String as) {
		return ((runtime.h_Native.strlen(as)==1)&&(((as).compareTo("0")>=0)&&((as).compareTo("9")<=0)));
	}
	boolean f_isDigits(String as) {
		return f_isDigits0(as, 0, runtime.h_Native.strlen(as));
	}
	boolean f_isDigits0(String as, int aindex, int alen) {
		if ((aindex<alen)) {
			return (f_isDigit(runtime.n_getCharAt.invoke(as, ((Integer)aindex)))&&f_isDigits0(as, (aindex+1), alen));
		} else {
			return (alen>0);
		}
	}
	boolean f_isDouble(String as0) {
		final String l0_s = f_ltrim2(f_trim(as0), "-");
		final Object[] l1_arr = f_strSplit(l0_s, ".");
		boolean l4_$3;
		if (((runtime.h_Native.strlen(l0_s)>0)&&(runtime.h_Native.length(l1_arr)<3))) {
			final Func1<Boolean,String> l6_$5 = new Func1<Boolean,String>() {
				final public Boolean invoke(final String ael) {
					return ((Boolean)f_isDigits(ael));
				}
			};
			l4_$3=runtime.m_runtime.f_all(runtime.h_Native.map(l1_arr, ((Func1<Object,Object>)(Func1)l6_$5)));
		} else {
			l4_$3=false;
		}
		return l4_$3;
	...
	Object[] f_strSplit(String as, String asep) {
		if ((asep).equals("")) {
			return (new Object[] { as });
		} else {
			return runtime.h_Native.list2array(f_doStrSplit(as, asep, 0, runtime.m_list.f_makeList()));
		}
	}
